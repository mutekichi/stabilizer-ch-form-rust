...
fn compare_with_qiskit_references() {
    ...
    if !resources.exists() {
        println!("Skipping reference tests: '{}' directory not found.", TEST_DATA_DIR);
        return;
    }
    ...
    for entry in fs::read_dir(resources).expect("Failed to read test resources directory") {
        ...
        if path.is_dir() {
            ...
            assert!(qasm_path.exists(), "circuit.qasm not found in {}", test_case_name);
            ...
            let ch_form = StabilizerCHForm::try_from(&circuit)
                .unwrap_or_else(|e| panic!("Failed to convert to CH-form for {}: {}", test_case_name, e));
            
            let our_statevector = ch_form.to_statevector();

            let reference_statevector = common::load_statevector_from_file(&sv_path)
                .unwrap_or_else(|e| panic!("Failed to load reference statevector for {}: {}", test_case_name, e));
